import { DigitalaxLookNFT as DigitalaxLookNFTContract, Transfer } from "../generated/DigitalaxLookNFT/DigitalaxLookNFT";
...
export function handleTransfer(event: Transfer): void {
  ...
  if (event.params.from.equals(ZERO_ADDRESS)) {
    ...
    digitalaxLookNft.background = contract.getBackgroundColour(event.params.tokenId);
    ...
    let digitalaxLookNft = DigitalaxLookNFT.load(event.params.tokenId.toString());
    if (digitalaxLookNft) {
      let owner = contract.try_ownerOf(event.params.tokenId);
      if (!owner.reverted) {
        digitalaxLookNft.owner = owner.value;
