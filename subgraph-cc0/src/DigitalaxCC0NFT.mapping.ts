...
import {
  ...
  DigitalaxCC0NFT as DigitalaxCC0NFTContract,
  TokenPrimarySalePriceSet,
} from "../generated/DigitalaxCC0NFT/DigitalaxCC0NFT";
...
import { loadOrCreateCC0Resident } from "./factory/DigitalaxCC0Designer.factory";
import { loadOrCreateDigitalaxCC0Collector } from "./factory/DigitalaxCollectorV2.factory";

import { ZERO_ADDRESS } from "./constants";
import { loadOrCreateDigitalaxCC0Children } from "./factory/DigitalaxCC0NFTChild.factory";
...
export function handleTransfer(event: Transfer): void {
  ...
  if (event.params.from.equals(ZERO_ADDRESS)) {
    ...
    let garment = new DigitalaxCC0NFT(garmentId);
    let garmentDesigner = loadOrCreateCC0Resident(garmentId);
    ...
    garment.resident = garmentDesigner.id;
                ...
                for (let i = 0; i < attributes.length; i += 1) {
                  if (attributes[i].kind == JSONValueKind.OBJECT) {
                    ...
                    let garmentAttribute = new GarmentAttribute(
                      "digitalaxCC0-" + garment.id + i.toString()
                    );
    ...
    let collector = loadOrCreateDigitalaxCC0Collector(event.params.to);
  ...
  // handle burn
    ...
    store.remove("DigitalaxCC0NFT", event.params.tokenId.toString());
  }
  // just a transfer
  else {
    // Update garment info
    let garment = DigitalaxCC0NFT.load(event.params.tokenId.toString());
    if (garment) {
      let owner = contract.try_ownerOf(event.params.tokenId);
      if (!owner.reverted) {
        garment.owner = owner.value;
      ...
      let fromCollector = loadOrCreateDigitalaxCC0Collector(event.params.from);
      ...
      let toCollector = loadOrCreateDigitalaxCC0Collector(event.params.to);
...
export function handleChildReceived(event: ReceivedChild): void {
  ...
  let garment = DigitalaxCC0NFT.load(event.params.toTokenId.toString());

  let child = loadOrCreateDigitalaxCC0Children(
    event,
...
export function handleUriUpdated(event: DigitalaxGarmentTokenUriUpdate): void {
  let contract = DigitalaxCC0NFTContract.bind(event.address);
  let garment = DigitalaxCC0NFT.load(event.params._tokenId.toString());
  if (garment == null) {
    garment = new DigitalaxCC0NFT(event.params._tokenId.toString());
    garment.resident = contract
