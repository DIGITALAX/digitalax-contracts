...
import {
  ...
  DigitalaxF3MNFT as DigitalaxF3MNFTContract,
  TokenPrimarySalePriceSet,
} from "../generated/DigitalaxF3MNFT/DigitalaxF3MNFT";
...
import { loadOrCreateF3MDesigner } from "./factory/DigitalaxF3MDesigner.factory";
import { loadOrCreateDigitalaxF3MCollector } from "./factory/DigitalaxCollectorV2.factory";

import { ZERO_ADDRESS } from "./constants";
import { loadOrCreateDigitalaxF3MChildren } from "./factory/DigitalaxF3MNFTChild.factory";
...
export function handleTransfer(event: Transfer): void {
  ...
  if (event.params.from.equals(ZERO_ADDRESS)) {
    ...
    let garment = new DigitalaxF3MNFT(garmentId);
    let garmentDesigner = loadOrCreateF3MDesigner(garmentId);
    ...
    let collector = loadOrCreateDigitalaxF3MCollector(event.params.to);
  ...
  // handle burn
    ...
    store.remove("DigitalaxF3MNFT", event.params.tokenId.toString());
  }
  // just a transfer
  else {
    // Update garment info
    let garment = DigitalaxF3MNFT.load(event.params.tokenId.toString());
    if (garment) {
      let owner = contract.try_ownerOf(event.params.tokenId);
      if (!owner.reverted) {
        garment.owner = owner.value;
      ...
      let fromCollector = loadOrCreateDigitalaxF3MCollector(event.params.from);
      ...
      let toCollector = loadOrCreateDigitalaxF3MCollector(event.params.to);
...
export function handleChildReceived(event: ReceivedChild): void {
  ...
  let garment = DigitalaxF3MNFT.load(event.params.toTokenId.toString());

  let child = loadOrCreateDigitalaxF3MChildren(
    event,
...
export function handleUriUpdated(event: DigitalaxGarmentTokenUriUpdate): void {
  let contract = DigitalaxF3MNFTContract.bind(event.address);
  let garment = DigitalaxF3MNFT.load(event.params._tokenId.toString());
  if (garment == null) {
    garment = new DigitalaxF3MNFT(event.params._tokenId.toString());
