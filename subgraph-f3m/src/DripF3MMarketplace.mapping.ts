import {ONE} from "../../subgraph-matic/src/constants";

import {
    OfferCreated,
    OfferPurchased,
    UpdateOfferPrimarySalePrice,
    DripF3MMarketplace as DripMarketplaceContract,
    OfferCancelled,
    DigitalaxMarketplaceContractDeployed,
    UpdateMarketplaceDiscountToPayInErc20
} from "../generated/DripF3MMarketplace/DripF3MMarketplace";

import {
    DripF3MMarketplaceOffer,
    DripF3MMarketplacePurchaseHistory,
...
export function handleOfferCreated(event: OfferCreated): void {
    let contract = DripMarketplaceContract.bind(event.address);
    let offer = new DripF3MMarketplaceOffer(event.params.garmentCollectionId.toString());
...
export function handleOfferPurchased(event: OfferPurchased): void {
    let contract = DripMarketplaceContract.bind(event.address);
    let collection = DigitalaxF3MNFTCollection.load(event.params.garmentCollectionId.toString());
    let history = new DripF3MMarketplacePurchaseHistory(event.params.bundleTokenId.toString());
    let offerData = contract.getOffer(event.params.garmentCollectionId);
