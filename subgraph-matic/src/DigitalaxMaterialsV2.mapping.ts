...
export function handleSingleTransfer(event: TransferSingle): void {
    ...
    let contract: DigitalaxMaterialsV2Contract = DigitalaxMaterialsV2Contract.bind(event.address);
    let childId: BigInt = event.params.id;
    let childToken: DigitalaxMaterialV2 | null = DigitalaxMaterialV2.load(childId.toString());
    ...
    if (!event.params.from.equals(ZERO_ADDRESS)) {
        ...
        for (let j: number = 0; j < totalChildOwned; j++) {
            ...
            if (childTokenId.toString() === compositeId) {
                ...
                let child = loadOrCreateDigitalaxChildV2Owner(event, event.params.from, id);
...
export function handleBatchTransfer(event: TransferBatch): void {
    ...
    for (let i: number = 0; i < totalIdsTransferred; i++) {
        ...
        let child: DigitalaxMaterialV2 | null = DigitalaxMaterialV2.load(childId.toString());
