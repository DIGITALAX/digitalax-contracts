...
import { DigitalaxSubscription, GarmentAttribute } from "../generated/schema";
...
export function handleChildReceived(event: ReceivedChild): void {
  ...
  let child = loadOrCreateDigitalaxSubscriptionChild(
    event,
...
export function handleTokenPriceSaleUpdated(
  ...
  let garment = DigitalaxSubscription.load(event.params._tokenId.toString());
  if (garment === null) {
    garment = new DigitalaxSubscription(event.params._tokenId.toString());
