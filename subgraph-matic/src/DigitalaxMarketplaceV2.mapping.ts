...
export function handleOfferCreated(event: OfferCreated): void {
    let contract = DigitalaxMarketplaceV2Contract.bind(event.address);
    let offer = new DigitalaxMarketplaceV2Offer(event.params.garmentCollectionId.toString());
...
export function handleOfferCancelled(event: OfferCancelled): void {
    let offer = DigitalaxMarketplaceV2Offer.load(event.params.bundleTokenId.toString());
