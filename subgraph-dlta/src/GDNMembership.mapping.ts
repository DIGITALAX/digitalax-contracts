...
import {
    Transfer,
    GDNMembership as GDNMembershipContract
} from "../generated/GDNMembership/GDNMembership";
...
import {loadOrCreateGDNMembershipCollector} from "./factory/GDNMembershipCollector.factory";
import {ZERO_ADDRESS} from "./constants";

export function handleTransfer(event: Transfer): void {
    log.info("Handle Garment Transfer @ Hash {}", [event.transaction.hash.toHexString()]);
    let contract = GDNMembershipContract.bind(event.address);

    // This is the birthing of a garment
    if (event.params.from.equals(ZERO_ADDRESS)) {
        let garmentId = event.params.tokenId.toString();
        let garment = new GDNMembership(garmentId);
        ...
        let collector = loadOrCreateGDNMembershipCollector(event.params.to);
    ...
    // handle burn
        ...
        store.remove('GDNMembership', event.params.tokenId.toString());
    }
    // just a transfer
    else {
        // Update garment info
        let garment = GDNMembership.load(event.params.tokenId.toString());
        let owner = contract.try_ownerOf(event.params.tokenId);
        if (!owner.reverted) {
            garment.owner = owner.value;
        }
        ...
        let fromCollector = loadOrCreateGDNMembershipCollector(event.params.from);
        ...
        let toCollector = loadOrCreateGDNMembershipCollector(event.params.to);
