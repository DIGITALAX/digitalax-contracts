...
import {loadOrCreateGDNDltaNFTCollector} from "./factory/GDNDltaNFTCollector.factory";
...
export function handleTransfer(event: Transfer): void {
    ...
    if (event.params.from.equals(ZERO_ADDRESS)) {
        ...
        let garment = new GDNDltaNFT(garmentId);
        ...
        let collector = loadOrCreateGDNDltaNFTCollector(event.params.to);
    ...
    // handle burn
        ...
        store.remove('GDNDltaNFT', event.params.tokenId.toString());
    }
    // just a transfer
    else {
        // Update garment info
        let garment = GDNDltaNFT.load(event.params.tokenId.toString());
        let owner = contract.try_ownerOf(event.params.tokenId);
        if (!owner.reverted) {
            garment.owner = owner.value;
        }
        ...
        let fromCollector = loadOrCreateGDNDltaNFTCollector(event.params.from);
        ...
        let toCollector = loadOrCreateGDNDltaNFTCollector(event.params.to);
