...
contract DigitalaxRootTunnel is BaseRootTunnel {
    ...
    */
    ...
    function _processMessageFromChild(bytes memory message) internal override {
        ...
        (_owners, _tokenIds, _primarySalePrices, _garmentDesigners, _tokenUris, _children, _childrenURIs, _childrenBalances) = abi.decode(message, (address[], uint256[], uint256[], address[], string[], uint256[][], string[][], uint256[][]));

        for( uint256 i; i< _tokenIds.length; i++){
            // With the information above, rebuild the 721 token on mainnet
            if(!nft.exists(_tokenIds[i])){
                uint256 newTokenId = nft.mint(_owners[i], _tokenUris[i], _garmentDesigners[i]); // TODO Check this with the matic way of doing mints (predicate)
                ...
                if(_children[i].length > 0){
                    for( uint256 j; j< _children[i].length; j++){
                        string memory singleURI = materials.uri(_children[i][j]);
                        if(bytes(singleURI).length < 1) {
                            ...
                            materials.mintChild(_children[i][j], _childrenBalances[i][j], address(nft), abi.encodePacked(newTokenId));
                        }
                    }
                }
            }
    ...
    function transferNFTsDataToMatic(uint256[] memory _tokenIds) external {
        ...
        // TODO For testing
       // bytes memory msg = abi.encode(_tokenIds, _salePrices, _designers, _tokenUris);
    }
}