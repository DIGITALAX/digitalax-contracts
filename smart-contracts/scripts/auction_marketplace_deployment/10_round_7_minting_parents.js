...
async function main() {
 ...
 // const testStartTime = '1606347000'; // 11/25/2020 @ 11:30pm (UTC) | 3:30pm pst November 25th only test
  ...
  const uris = [
      {
          // Auction 1 Exclusive
          uri: require('../../../../nft-minting-scripts/auction-metadata/token-data/parents/Among Us/Exclusive/CC Lockdown Skin/hash.json').uri,
          tokendIds: [100001],
          tokenAmounts: [2],
          price: reservePrice,
  ...
  for (let [index, auctionGarmentInfo] of uris.entries()) {
      ...
      const tx = await factory.mintParentWithChildren(
          ...
          '0x88BB4d01352C34dfd940Bd3f6f60B8EBd8e5C92b', // Who receives nft and can approve (beneficiary), should be deployer address
        );
