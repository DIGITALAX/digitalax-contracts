...
const ModelsCollection = artifacts.require('ModelsCollection');
const DigitalaxGarmentFactory = artifacts.require('DigitalaxGarmentFactory');
const ModelsMarketplace = artifacts.require('ModelsMarketplaceMock');
const ModelsMarketplaceReal = artifacts.require('ModelsMarketplace');
...
contract('ModelsMarketplace', (accounts) => {
  const [admin, smartContract, platformFeeAddress, minter, owner, designer, model, tokenBuyer, newRecipient, provider] = accounts;
  ...
  beforeEach(async () => {
    ...
    this.token = await ModelsNFT.new();
    ...
    this.garmentCollection = await ModelsCollection.new()
    ...
    this.marketplace = await ModelsMarketplace.new();
  ...
  describe('Admin functions', () => {
    beforeEach(async () => {
      await this.garmentCollection.mintCollection(randomTokenURI, designer, model, COLLECTION_SIZE, bundleID, 'Common', erc1155ChildStrandIds, amountsOfChildToken, {from: minter});
  ...
  describe('createOffer()', async () => {
    ...
    describe('validation', async () => {
      ...
      it('fails if token already has marketplace in play', async () => {
        ...
        await this.marketplace.createOffer(0,  ether('0.1'), '120',
            '1000000', '120', MAX_SIZE, [], [],  {from: minter});
      ...
      it('fails if you try to create an offer with a non minter address', async () => {
        ...
        await expectRevert(
           this.marketplace.createOffer('98', ether('0.05'), '120',
               '1000000', '120', MAX_SIZE, [], [],  {from: tokenBuyer}),
    ...
    describe('creating using real contract (not mock)', () => {
      it('can successfully create', async () => {
        const marketplaceNew = await ModelsMarketplaceReal.new();
  ...
  describe('buyOffer()', async () => {
    ...
    describe('try to buy offer', () => {
      ...
      beforeEach(async () => {
        await this.garmentCollection.mintCollection(randomTokenURI, designer, model, COLLECTION_SIZE, bundleID, 'Common', [], [], {from: minter});
        const garmentIds = await this.garmentCollection.getTokenIds(0);
        for (let i = 0; i < garmentIds.length; i ++) {
          await this.token.approve(this.marketplace.address, garmentIds[i], {from: minter});
        }
      ...
      it('transfer Mona only to the token creator and platform', async () => {
        ...
        expect(await this.monaToken.balanceOf(designer)).to.be.bignumber.equal(new BN('44000000000000000'));
        expect(await this.monaToken.balanceOf(model)).to.be.bignumber.equal(new BN('44000000000000000'));
    ...
    describe('try to buy offer', () => {
      ...
      beforeEach(async () => {
        await this.garmentCollection.mintCollection(randomTokenURI, '0x0000000000000000000000000000000000000000', model, COLLECTION_SIZE, bundleID, 'Common', [], [], {from: minter});
        const garmentIds = await this.garmentCollection.getTokenIds(0);
        for (let i = 0; i < garmentIds.length; i ++) {
          await this.token.approve(this.marketplace.address, garmentIds[i], {from: minter});
        }
      ...
      it('transfer Mona only to the token creator and platform', async () => {
        ...
        expect(await this.monaToken.balanceOf(model)).to.be.bignumber.equal(new BN('88000000000000000'));
    ...
    describe('try to buy offer', () => {
      ...
      beforeEach(async () => {
        await this.garmentCollection.mintCollection(randomTokenURI, designer, '0x0000000000000000000000000000000000000000', COLLECTION_SIZE, bundleID, 'Common', [], [], {from: minter});
        const garmentIds = await this.garmentCollection.getTokenIds(0);
        for (let i = 0; i < garmentIds.length; i ++) {
          await this.token.approve(this.marketplace.address, garmentIds[i], {from: minter});
        }
      ...
      it('transfer Mona only to the token creator and platform', async () => {
        ...
        // Validate that the garment owner/designer received FEE * (100% minus platformFEE of 12%)
        expect(await this.monaToken.balanceOf(model)).to.be.bignumber.equal(new BN('0'));
