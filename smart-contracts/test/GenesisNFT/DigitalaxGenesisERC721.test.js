...
const DigitalaxGenesisNFT = artifacts.require('DigitalaxGenesisNFTMock');
const ERC721ReceiverMock = artifacts.require('ERC721ReceiverMock');

contract('Core ERC721 behaviour tests for DigitalaxGenesisNFT', function ([owner, approved, anotherApproved, operator, other, multisig, buyer]) {

    const name = 'DigitalaxGenesis';
    const symbol = 'DXG';
    ...
    const ZERO_POINT_ONE_ETH = ether('0.1');

    beforeEach(async function () {
        this.accessControls = await DigitalaxAccessControls.new({from: owner});

        this.token = await DigitalaxGenesisNFT.new(
    ...
    describe('metadata', function () {
        ...
        describe('token URI', function () {
            beforeEach(async function () {
                await this.token.buy({from: owner, value: ZERO_POINT_ONE_ETH});
            });
            ...
            it('can be queried for an example token', async function () {
                expect(await this.token.tokenURI(firstTokenId)).to.be.equal(randomTokenURI);
            })

            it('can be queried for all tokens', async function() {
                expect(await this.token.tokenURI_()).to.be.equal(randomTokenURI);
            })
        });
    });
    ...
    context('with minted tokens', function () {
        beforeEach(async function () {
            ...
            // outside the genesis sale window for transfers
                ...
                context('when sent to the owner', function () {
                    ...
                    it('keeps same tokens by index', async function () {
                        ...
                        const tokensListed = await Promise.all(
                            [0].map(i => this.token.tokenOfOwnerByIndex(owner, i)),
                        );
    ...
    describe('_mint(address, uint256)', function () {
        context('with minted token', async function () {
            beforeEach(async function () {
                ({ logs: this.logs } = await this.token.buy({from: owner, value: ZERO_POINT_ONE_ETH}));
            });
