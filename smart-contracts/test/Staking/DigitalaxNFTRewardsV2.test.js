...
contract('DigitalaxNFTRewardsV2', (accounts) => {
  ...
  beforeEach(async () => {
    ...
    this.nft = await DigitalaxGarmentNFTv2.new();
...
/*
  describe('Set Mona Staking contract', () => {
    describe('setNftStaking()', () => {
      it('fails when not admin', async () => {
        await expectRevert(
            this.digitalaxRewards.setNftStaking(constants.ZERO_ADDRESS, {from: staker}),
            'DigitalaxRewardsV2.setNftStaking: Sender must be admin'
        );
      });

      it('successfully updates mona staking', async () => {
        const original = await this.digitalaxRewards.nftStaking();
        expect(original).to.be.equal(this.nftStaking.address);

        await this.digitalaxRewards.setNftStaking(this.accessControls.address, {from: admin});

        const updated = await this.digitalaxRewards.nftStaking();
  ...
  describe('Initialize Pools', () => {
    ...
    describe('initializePools()', () => {
      ...
      it('fails when not admin', async () => {
        await expectRevert(
            this.digitalaxRewards.initializePools(1, [1], [1], [1], {from: staker}),
      ...
      it('fails when arrays are not proper length', async () => {
        await expectRevert(
            this.digitalaxRewards.initializePools(1, [1], [1, 2], [1],{from: admin}),
            'DigitalaxRewardsV2.initializePools: Please check weeks and weight point revenue lengths'
        );
        await expectRevert(
            this.digitalaxRewards.initializePools(1, [1], [1], [1, 2], {from: admin}),
      ...
      it('fails when it is empty array', async () => {
        await expectRevert(
            this.digitalaxRewards.initializePools(0, [], [], [], {from: admin}),
            'DigitalaxRewardsV2.initializePools: Must be 1 or more weeks'
        );
      });

      it('successfully initializes pool', async () => {
        await this.digitalaxRewards.initializePools(1, [1], [100], [10], {from: admin});
        ...
        const _bonusWeightPointsRevenueSharing = await this.digitalaxRewards.getBonusMonaWeightPoints(1, 1);
        expect(_bonusWeightPointsRevenueSharing).to.be.bignumber.equal('10');
  ...
  describe('Update Rewards', () => {
    ...
    describe('updateRewards()', () => {
      it('successfully updates rewards', async () => {
        ...
        const nftStakingBalanceTracker = await balance.tracker(this.nftStaking.address, 'ether');
        const originalNftStakingContractBalance = await nftStakingBalanceTracker.get('wei');

        expect(originalNftStakingContractBalance).to.be.bignumber.equal(new BN('0'));
  ...
  describe('Gets staked mona', () => {
    beforeEach(async () => {
      ...
      await this.nftStaking.initNftStakingPool(
          1,
          TWO_ETH,
          THREE_ETH,
          100,
      ...
      await this.monaToken.approve(this.nftStaking.address, TEN_ETH, {from: staker});
      await this.monaToken.approve(this.nftStaking.address, TEN_ETH, {from: newRecipient});
      await this.nftStaking.stake(0, TWO_ETH, {from: staker});
      await this.nftStaking.stake(0, THREE_ETH, {from: newRecipient});
      await this.monaToken.approve(this.digitalaxRewards.address, TEN_ETH.mul(new BN('5')), {from: admin});
      await this.digitalaxRewards.depositRevenueSharingRewards(1, TEN_ETH, TEN_ETH, {from: admin});
      await this.digitalaxRewards.depositRevenueSharingRewards(2, TEN_ETH, TEN_ETH, {from: admin});
