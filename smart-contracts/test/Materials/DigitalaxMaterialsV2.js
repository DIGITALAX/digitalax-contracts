...
contract('DigitalaxMaterials 1155 behaviour tests', function ([admin, minter, tokenHolder, tokenBatchHolder, smart_contract, random, ...otherAccounts]) {
  ...
  describe('mintChild()', function() {
    ...
    it('Can successfully mint then burn batch', async function() {
      await this.token.mintChild(STRAND_ONE_ID, '4', tokenHolder, web3.utils.encodePacked(''), {from: smart_contract});
      expect(await this.token.balanceOf(tokenHolder, STRAND_ONE_ID)).to.be.bignumber.equal('4');
      await this.token.setGarmentNFTApproved(random, {from: admin});
      await this.token.burnBatch(tokenHolder, [STRAND_ONE_ID], [4], {from: random});
    });
  ...
  describe('batchMintChildren()', function() {
    ...
    it('Reverts when any amount is zero', async function() {
      ...
      await expectRevert(
        this.token.batchMintChildren(['100001'], ['0'], tokenBatchHolder, web3.utils.encodePacked(''), {from: smart_contract}),
    ...
    it('Reverts when array lengths are inconsistent', async function() {
      await expectRevert(
        this.token.batchMintChildren(['100001'], [], tokenBatchHolder, web3.utils.encodePacked(''), {from: smart_contract}),
