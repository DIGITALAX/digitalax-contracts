...
const DigitalaxIndex = artifacts.require('DigitalaxIndex');
const DigitalaxWhitelistedSales = artifacts.require('DigitalaxWhitelistedSalesMock');
...
contract('DigitalaxMarketplaceV2', (accounts) => {
  ...
  beforeEach(async () => {
    ...
    this.index = await DigitalaxIndex.new(
        this.accessControls.address,
      { from: minter }
    );
    ...
    this.marketplace = await DigitalaxMarketplaceV2.new();
    ...
    await this.oracle.pushReport(EXCHANGE_RATE, {from: provider});
     ...
     this.whitelistedSales = await DigitalaxWhitelistedSales.new();
    await this.whitelistedSales.initialize(
      this.accessControls.address,
      this.garmentCollection.address,
    ...
    await this.accessControls.addMinterRole(this.whitelistedSales.address, {from: admin});
    await this.accessControls.addSmartContractRole(this.whitelistedSales.address, {from: admin});
    await this.accessControls.addAdminRole(this.whitelistedSales.address, {from: admin});
  ...
  describe('createOffer()', async () => {
    ...
    describe('creating using real contract (not mock)', () => {
      it('can successfully create', async () => {
        const marketplaceNew = await DigitalaxMarketplaceV2Real.new();
  ...
  describe('buyOffer()', async () => {
    describe('try to buy offer', () => {
      beforeEach(async () => {
        ...
        //   await this.token.approve(this.marketplace.address, garmentIds[i], {from: minter});
        // }
        await this.whitelistedSales.addWhitelister(designer, {from: admin});
        await this.marketplace.setNowOverride('100');
        await this.whitelistedSales.setNowOverride('100');
        // await this.marketplace.createOffer(
        //   0, // ID
        //   ether('0.1'),
        //   '120',
        ...
        //   {from: minter}
        // );
        await this.whitelistedSales.mintAndList(randomTokenURI, 20, [],  ether('0.1'), {from: designer});

      });

      it('buys the offer', async () => {
        await this.marketplace.setNowOverride('120');
        await this.whitelistedSales.setNowOverride('120');
        ...
        expect(_startTime).to.be.bignumber.equal('101');
        expect(_availableAmount).to.be.bignumber.equal('19');
        expect(_platformFee).to.be.bignumber.equal('150');
        expect(_discountToPayERC20).to.be.bignumber.equal('0');
      });
      ...
      it('transfer Mona only to the token creator and platform', async () => {
        ...
        expect(await this.monaToken.balanceOf(designer)).to.be.bignumber.equal(new BN('85000000000000000'));

        // Validate that the treasury wallet (platformFeeRecipient) received platformFee minus discount for paying in Mona
        expect(await this.monaToken.balanceOf(platformFeeAddress)).to.be.bignumber.equal(new BN('15000000000000000'));
  ...
  describe('reclaimERC20()', async () => {
    describe('validation', async () => {
      it('cannot reclaim erc20 if it is not Admin', async () => {
        await expectRevert(
          this.whitelistedSales.reclaimERC20(this.weth.address, {from: tokenBuyer}),
      ...
      it('can reclaim Erc20', async () => {
        ...
        await this.weth.transfer(this.whitelistedSales.address, TWENTY_TOKENS, { from: minter });

        const adminBalanceBeforeReclaim = await this.weth.balanceOf(admin);
        expect(await this.weth.balanceOf(this.whitelistedSales.address)).to.be.bignumber.equal(TWENTY_TOKENS);

        // Reclaim erc20 from contract
        await this.whitelistedSales.reclaimERC20(this.weth.address, {from: admin});

        expect(await this.weth.balanceOf(this.whitelistedSales.address)).to.be.bignumber.equal(new BN('0'));
