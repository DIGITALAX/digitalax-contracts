...
import {
  ...
  DigitalaxHimeNFT as DigitalaxHimeNFTContract,
  TokenPrimarySalePriceSet,
} from "../generated/DigitalaxHimeNFT/DigitalaxHimeNFT";
...
import { loadOrCreateDigitalaxHimeChildren } from "./factory/DigitalaxHimeNFTChild.factory";
...
export function handleTransfer(event: Transfer): void {
  ...
  if (event.params.from.equals(ZERO_ADDRESS)) {
    ...
    let garment = new DigitalaxHimeNFT(garmentId);
    let garmentDesigner = loadOrCreateHimeDesigner(garmentId);
                ...
                for (let i = 0; i < attributes.length; i += 1) {
                  if (attributes[i].kind == JSONValueKind.OBJECT) {
                    ...
                    let garmentAttribute = new GarmentAttribute(
                      "digitalaxHime-" + garment.id + i.toString()
                    );
    ...
    let collector = loadOrCreateDigitalaxHimeCollector(event.params.to);
  ...
  // handle burn
    ...
    store.remove("DigitalaxHimeNFT", event.params.tokenId.toString());
  }
  // just a transfer
  else {
    // Update garment info
    let garment = DigitalaxHimeNFT.load(event.params.tokenId.toString());
    if (garment) {
      let owner = contract.try_ownerOf(event.params.tokenId);
      if (!owner.reverted) {
        garment.owner = owner.value;
      ...
      let fromCollector = loadOrCreateDigitalaxHimeCollector(event.params.from);
      ...
      let toCollector = loadOrCreateDigitalaxHimeCollector(event.params.to);
...
export function handleChildReceived(event: ReceivedChild): void {
  ...
  let garment = DigitalaxHimeNFT.load(event.params.toTokenId.toString());

  let child = loadOrCreateDigitalaxHimeChildren(
    event,
...
export function handleUriUpdated(event: DigitalaxGarmentTokenUriUpdate): void {
  let contract = DigitalaxHimeNFTContract.bind(event.address);
  let garment = DigitalaxHimeNFT.load(event.params._tokenId.toString());
  if (garment == null) {
    garment = new DigitalaxHimeNFT(event.params._tokenId.toString());
