...
} from '../generated/DigitalaxHimeIndex/DigitalaxHimeIndex'

import {
  DigitalaxDesigner,
  DigitalaxHimeCollectionGroup,
  DigitalaxDeveloper,
...
export function handleCollectionGroupAdded(event: CollectionGroupAdded): void {
  ...
  let collectionGroup = new DigitalaxHimeCollectionGroup(
...
export function handleCollectionGroupRemoved(
  ...
  store.remove('DigitalaxHimeCollectionGroup', event.params.sid.toString())
}
...
export function handleCollectionGroupUpdated(
  ...
  let collectionGroupId = event.params.sid
  let collectionGroup = DigitalaxHimeCollectionGroup.load(
    collectionGroupId.toString(),
  )
  if (collectionGroup) {
    let auctions = new Array<string>()
...
export function handleDesignerGroupRemoved(event: DesignerGroupRemoved): void {
  ...
  if (designer) {
    ...
    for (let i = 0; i < collectionIds.length; i += 1) {
      ...
      let collection = DigitalaxHimeNFTCollection.load(collectionId.toString())
      if (collection) {
    ...
    for (let i = 0; i < auctionIds.length; i += 1) {
      ...
      let auction = DigitalaxHimeAuction.load(auctionId.toString())
      if (auction) {
...
export function handleDeveloperGroupRemoved(
  ...
  if (developer) {
    ...
    // for (let i = 0; i < collectionIds.length; i += 1) {
    //   let collectionId = collectionIds[i];
    //   let collection = DigitalaxHimeNFTCollection.load(collectionId);
    //   if (collection) {
    ...
    // for (let i = 0; i < auctionIds.length; i += 1) {
    //   let auctionId = auctionIds[i];
    //   let auction = DigitalaxHimeAuction.load(auctionId);
    //   if (auction) {
